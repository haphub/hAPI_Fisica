...
float b_linear = 3;                   // Linear damping in N/m
...
float damping = -b_linear * dxh_filt;
float force_tot= force+damping; 

  // Section 3: transforms force to torque at motor
  Tp = rp/rs * rh * (force_tot); // compute the required motor pulley torque (Tp) to generate that force
  
  forceOutput(mtr, force_tot, Tp);
}
